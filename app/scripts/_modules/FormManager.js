...
SuperSnooper.Modules.FormManager = function() {
    ...
    //Reset the scroll on our input areas when we blur them...
    ...
    for(var i = 0; i < this.inputItems.length; i++) {
        ...
        //Key up/down event
        //_item.on('keyup', function(_item) { this.inputItemCheck(_item); }.bind(this, _item));
        //_item.on('keydown', function(_item) { this.inputItemCheck(_item); }.bind(this, _item));
...
SuperSnooper.Modules.FormManager.prototype.inputItemFocus = function(_item, _focus) {
    if(!_focus) {
        ...
        if(_item.html() === _item.attr('data-default')) {
            //Text is the default, so focus it after a little delay
            setTimeout(function() {
...
//  EDIT THE KEYWORDS
//--------------------------------------------------------------------------
SuperSnooper.Modules.FormManager.prototype.editKeywords = function() {
    //Create the content
    var _content = SuperSnooper.templates.keywords({});

    //Open the lightbox
    SuperSnooper.Signals.lightbox.dispatch('open', {type:'keywords', content:_content});
...
//  ADD A KEYWORD
//--------------------------------------------------------------------------
SuperSnooper.Modules.FormManager.prototype.addKeyword = function(_master, _variant, _id) {
    //Vars
    _master = _master || '';
    _variant = _variant || '';
...
//--------------------------------------------------------------------------
SuperSnooper.Modules.FormManager.prototype.setKeywordRowPositions = function() {
    //Vars
    for(var i=0;i<this.keywords.length;i++) {
...
//  UPDATE THE KEYWORDS
//--------------------------------------------------------------------------
SuperSnooper.Modules.FormManager.prototype.updateKeywords = function(_closeLightBox) {
    //Close?
    ...
    if(_closeLightBox) {
        SuperSnooper.Signals.lightbox.dispatch('close', {clear:false});
    }
};
...
//--------------------------------------------------------------------------
SuperSnooper.Modules.FormManager.prototype.setKeywordCounts = function() {
    //Count
    var _variantCount = 0;
    for(var i=0;i<this.keywords.length;i++) {
